...
public class Lotto5App {
    ...
    public static void main(String[] args) {
        try (Scanner sc = new Scanner(new File("C:/jtmp/lotto5in.txt"));
             PrintStream ps = new PrintStream("C:/jtmp/lotto5out.txt", StandardCharsets.UTF_8)) {
            ...
            while ((num = sc.nextInt()) != -1 && pivot <= 48) {
                inputNumbers[pivot++] = num;
                // pivot++
            }

            int[] numbers = Arrays.copyOfRange(inputNumbers, 0, pivot); //αντιγράφει μέχρι pivot - 1
            Arrays.sort(numbers);
                        ...
                        for (int l = k + 1; l <= window + 3; l++) {
                            for (int m = l + 1; m <= window + 4; m++) {
                                ...
                                if (!isEvenGE(result, 3) && !isOddGE(result, 3)) {
                                    ps.printf("%d %d %d %d %d%n", result[0], result[1], result[2], result[3], result[4]);
                                }
                            }
                        }
                    }
     ...
     * @return                  true, if the number of evens is greater than
     *                          or equal to a threshold limit, false otherwise.
     */
